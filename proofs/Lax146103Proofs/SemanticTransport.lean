...
theorem realize_equiv (e : M ≃[L] N) :
    {n m : Nat} → (phi : Lax146103.MSOSyntax.Formula L n m) →
      (v : Fin n → M) → (V : Fin m → Set M) →
      Lax146103.MSOSemantics.Realize phi (e ∘ v) (fun X => e '' V X) ↔
        Lax146103.MSOSemantics.Realize phi v V
  | _, _, .falsum, _, _ => Iff.rfl
  | _, _, .equal t₁ t₂, v, _ => by
      simp only [Lax146103.MSOSemantics.Realize, HomClass.realize_term]
  ...
  | _, _, .or phi psi, v, V => by
      simp only [Lax146103.MSOSemantics.Realize]
  ...
  | _, _, .exFO phi, v, V => by
      ...
      · rintro ⟨y, hy⟩
        refine ⟨e.symm y, ?_⟩
        have hv : Lax146103.MSOSemantics.consVal y (e ∘ v) =
            e ∘ Lax146103.MSOSemantics.consVal (e.symm y) v := by
          funext i
          refine Fin.cases ?_ (fun j => ?_) i
          · simp [Lax146103.MSOSemantics.consVal]
          · rfl
        rw [hv] at hy
        exact (realize_equiv e phi _ V).mp hy
      · rintro ⟨x, hx⟩
        refine ⟨e x, ?_⟩
        have hv : Lax146103.MSOSemantics.consVal (e x) (e ∘ v) =
            e ∘ Lax146103.MSOSemantics.consVal x v := by
          funext i
  ...
  | _, _, .exSO phi, v, V => by
      ...
      · rintro ⟨Y, hY⟩
        let X : Set M := e ⁻¹' Y
        refine ⟨X, ?_⟩
        have hsets : Lax146103.MSOSemantics.consVal Y (fun Z => e '' V Z) =
            fun Z => e '' Lax146103.MSOSemantics.consVal X V Z := by
          funext i
      ...
      · rintro ⟨X, hX⟩
        refine ⟨e '' X, ?_⟩
        have hsets : Lax146103.MSOSemantics.consVal (e '' X) (fun Z => e '' V Z) =
            fun Z => e '' Lax146103.MSOSemantics.consVal X V Z := by
          funext i
          refine Fin.cases rfl (fun _ => rfl) i
        rw [hsets]
